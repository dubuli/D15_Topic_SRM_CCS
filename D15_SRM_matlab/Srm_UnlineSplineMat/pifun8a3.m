% 根据7a3改编写
function [sys,x0,str,ts,simStateCompliance] = pifun8a3(t,x,u,flag)
...
iDesInt_global=iDesInt_global+(u(1)-u(2))/30;
...
if u(2)>3000
    if u(1)>u(2)
        if iDesInt_global>iDesIntUp_global+200
            iDesInt_global=iDesIntUp_global+200;
        end
    end
end
...
if u(2)>3000
    if u(1)>u(2)
        if iDes_global>iDesIntUp_global+650
            iDes_global=iDesIntUp_global+650;
        end
    end
end
...
thetadvance=thetadvance*0.8+thetadvancetemp*0.2;
...
if u(2)>7500
    thetaon_apc=thetaon_apc+(u(1)-u(2))/10000;
elseif u(2)<7500
...
if thetaon_apc>(7-thetadvance)          %让开通角在-1到10之间
    thetaon_apc=7-thetadvance;
elseif thetaon_apc<(-4-thetadvance)
    thetaon_apc=-4-thetadvance;
end
