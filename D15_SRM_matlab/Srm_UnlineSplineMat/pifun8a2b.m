% 根据7a3改编写
function [sys,x0,str,ts,simStateCompliance] = pifun8a2b(t,x,u,flag)
...
iDesInt_global=iDesInt_global+(u(1)-u(2))/2;
...
if u(2)>3000
    if u(1)>u(2)
        if iDesInt_global>iDesIntUp_global*2
            iDesInt_global=iDesIntUp_global*2;
        end
    end
end
...
if u(2)>3000
    if u(1)>u(2)
        if iDes_global>iDesIntUp_global*2
            iDes_global=iDesIntUp_global*2;
        end
    end
end
...
thetaon_apc_int=thetaon_apc_int+(u(1)-u(2))/5000;
if u(2)>6500
    thetaon_apc=thetaon_apc_int+(u(1)-u(2))/100;
    iDesInt_global=iDesInt_global*1.01;
elseif u(2)<6500
...
if thetaon_apc>(8-thetadvance)          %让开通角在-1到10之间
