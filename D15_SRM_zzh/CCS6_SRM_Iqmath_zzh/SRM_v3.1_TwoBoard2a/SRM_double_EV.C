...
Uint32 fation1,fation2;     //·½ÏòÅÌËÙ¶Èµ÷½Ú
...
void main(void)
{
  ...
  Setup_GpioSet() ; //´Ëº¯ÊýÅÔÚ¿ªÍ·£¬ÒòÎªMUXµÄ¸Ä±ä»á¸´Î»Ä³Ð©¼Ä´æÆ÷
...
//  START_CpuTmr0() ;  //ÓÃµ½¶¨Ê±Æ÷T0µÄÆäËû¼Ä´æÆ÷³õÊ¼»¯ºó£¬ÔÙÆô¶¯T0
...
//  pid1_spd.Ki = _IQ(T*SpeedLoopPrescaler);							
 ...
 // Delay_msec(800);
  ...
  ADC_START ;
//  ileg3_vdc1.read(&ileg3_vdc1);  //½øÈëTOÖÐ¶ÏÖ®Ç°¬ÏÈ½øÐÐÒ»´çÑ¹µçÁ÷²ÉÑù£¬·ÀÖ¹Îó¶¯×÷
  ...
  //LCDPO_PutStr("on =    off=")  ;
       ...
       LCDPO_SetColor(RED,BLUE);                 //ºìÉ«×ÖÌå£¬À¶É«±³¾°
       LCD_PutStr(0x55,180,40,"Ë«ÂÖ¶ÀÁ¢Çý¶¯µç¶¯Æû³µÊÔÑéÆ½Ì¨") ;
       LCDPO_DispClock(300,550);                 //ÏÔÊ¾Ê±ÖÓ
       LCD_PutStr(0x54,10,100,"¼×µç»ú²ÎÊýÏÔÊ¾") ;   
       LCD_PutStr(0x54,400,100,"ÒÒµç»ú²ÎÊýÏÔÊ¾") ;
       LCDPO_SetColor(WHITE,BLUE);               //°×É«×ÖÌå£¬À¶É«±³¾°
  ...
  {    
    ...
    {
//*************************************
// cmd:0x53 ÏÔÊ¾8*8µãÕóASCII×Ö·û´®
//     0x54 ÏÔÊ¾16*16µãÕóºº×Ö´®£¨GBK£© 
//	   0x55 ÏÔÊ¾32*32µãÕóºº×Ö´®£¨GB2312£©
//	   0x6E ÏÔÊ¾12*12µãÕóºº×Ö´®£¨GBK£© 
//     0x6F ÏÔÊ¾24*24µãÕóºº×Ö´®£¨GB2312£©
// y:ºá×ø±ê
// x:×Ý×ø±ê
//str:ÒªÏÔÊ¾µÄ×Ö·û´®
//*************************************
