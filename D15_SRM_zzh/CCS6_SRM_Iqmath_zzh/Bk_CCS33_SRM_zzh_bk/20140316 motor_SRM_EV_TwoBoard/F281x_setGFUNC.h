...
void Setup_PCLKCRs(void)
{
  ...
  SysCtrRegs.HISPCP.all = 0x0001 ;
  SysCtrRegs.LOSPCP.all = 0x0002 ;
...
void Setup_CpuCtrl()
{
  ...
  // required.  For Rev C and after this is no longer required
   ...
   Setup_PLLclks(10) ;   // System Clock = 15MHz*10/2 = 75MHz 
...
// SCI(UART) init.
	...
	SciAstRegs.SCICTL2.bit.RXBKINTENA =1;	// RX/BK INT ENA=1,
	SciAstRegs.SCICTL2.bit.TXINTENA =1;	// TX INT ENA=1,
...
void Setup_FlashRM(void)
{
   ...
   Flash_Regs.FOPT.bit.ENPIPE = 1;  //流水线操作模式使能
