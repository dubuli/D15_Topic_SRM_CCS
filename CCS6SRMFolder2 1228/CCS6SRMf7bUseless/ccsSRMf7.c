...
//		KickDog();
		...
		if(SRM.position_state<1 || SRM.position_state>6)	{
...
//		while(SWITCH1==1 || PAOCUP==1 ||PAOCDN==1 || PBOCUP==1 || PBOCDN==1 || PCOCUP==1 ||PCOCDN==1)
		...
		/*----------------------*/
		testzero=88/testzero;
		testzero=3322;
		testzero=0.001*testzero;	//235 cycles
		testzero=99;				//1
		testzero=testzero>>4;		//3
		testzero=testzero*16;		//2
		testzero=testzero/16;		//6
		testzero=testzero/2;		//5
		testzero=testzero<<3<<16;	//4
		testzero=testzero>>19;		//3
		testzero=testzero<<19;		//3

		testzero=test2;				//testzero=2;(float test2=2.3)
		SRM.integral_speed_error=(long)CURRENT_1A*2<<16;	//3 cycles
		...
		//speedpiFlag is 1KHz;
		SRM.integral_speed_error=SRM.integral_speed_error+4/30*136*speed_error;
		SRM.integral_speed_error=SRM.integral_speed_error+136*speed_error*4/32;	//18cycles
