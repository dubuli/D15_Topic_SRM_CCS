...
void main(void)
 {
	...
	SRM.wDes_10xrpm=10000;	//Desninate rpm
/*---------------------------------**
** 	attention                      **
...
//ADC------------------------------------------
	...
	//	if(SWITCH1==1 || PAOCUP==1 ||PAOCDN==1 || PBOCUP==1 || PBOCDN==1 || PCOCUP==1 ||PCOCDN==1 || WatchdogFlag==0)	{
		...
		else if(SRM.iFB[0]>CURRENT_1A*15 || SRM.iFB[1]>CURRENT_1A*15 || SRM.iFB[2]>CURRENT_1A*15)	{
...
/*		if(SRM.position_state==1 && EvbRegs.ACTRB.bit.CMP8ACT==3 && StartFlag) {
		...
		if(SpeedPiFlag==1 && StartFlag==1)	{
			...
			SRM.integral_speed_error=SRM.integral_speed_error+speed_error*14*KI;	//136*480=65536  //KI=1  KI/32768*1000=1/32,32768=1<<15
			if(SRM.integral_speed_error>((long)CURRENT_1A<<16)*15)	{		//>>3,8A
				SRM.integral_speed_error=((long)CURRENT_1A<<16)*15;
			}
			else if(SRM.integral_speed_error<-(long)CURRENT_1A<<3<<16)	{
				SRM.integral_speed_error=-(long)CURRENT_1A<<3<<16;
			}

			if( (speed_error*KP) > (long)CURRENT_1A*9) {
				iDes= (long)CURRENT_1A*9 +(SRM.integral_speed_error>>16);	//KP=10KP注意速度已经是10倍速度 ,KI
			}
			else {
				iDes=speed_error*KP+(SRM.integral_speed_error>>16);	//KP=10KP注意速度已经是10倍速度 ,KI
			}

			if(iDes>CURRENT_1A*15)	{
				iDes=CURRENT_1A*15;
			}
			else if(iDes<0)	{
				iDes=0;
...
void PhaseControl(anSRM_struct *anSRM)	//int the ADC interrupt
{
	...
	for (phase = 0; phase< NUMBER_OF_PHASES; phase++)
	{
		...
		else if ((angle >= (182*8)) && (angle < 182*135))
		{
			anSRM->active[phase] = 1;
			temp = 0x1 << phase;
			anSRM->iDes[phase] = iDes;//Important!!
