...
void main(void)
{
	...
	SRM.wDes_10xrpm=8000;	//Desninate rpm
/*---------------------------------**
** 	attention                      **
...
/*		if(SRM.position_state==1 && EvbRegs.ACTRB.bit.CMP8ACT==3 && StartFlag) {
		...
		if(SpeedPiFlag==1 && StartFlag==1)	{
			...
			iDes=speed_error*1/10 +(SRM.integral_speed_error>>16);	//KP=10KP注意速度已经是10倍速度 ,KI
...
//		}
		if(!((count+10)%480))		{				//changeto 9600Hz 480-20Hz			//%250 20Hz		//5Hz// 5000/100=50 Hz
			GpioDataRegs.GPEDAT.bit.GPIOE1=0;
			speedoutput=SRM.wEst_10xrpm;

			if((SciaTx_Ready() == 1) )//&& (SendFlag == 1))
				//SciTemp=SRM.wEst_10xrpm;
				SciaRegs.SCITXBUF=(speedoutput >> 8 ) & 0xff;// & 0xff);
		}
		...
		if(!((count)%480))		{				//changeto 9600Hz 480-20Hz			//%250 20Hz		//5Hz// 5000/100=50 Hz
			...
			if((SciaTx_Ready() == 1) )//&& (SendFlag == 1))
				//SciTemp=SRM.wEst_10xrpm;
				SciaRegs.SCITXBUF=((speedoutput) & 0xff );// & 0xff);
		}
...
/* 	changed in 2812f5	.MYWORK	 */
	...
	anSRM->trans_lut[2][2].position = (WORD) PIBYSIX_16 * 11;
	anSRM->trans_lut[3][3].position = PIBYSIX_16;
	anSRM->trans_lut[4][3].position = (WORD) PIBYSIX_16 * 7;
	anSRM->trans_lut[5][2].position = PIBYSIX_16 * 5;
	anSRM->trans_lut[6][1].position = (WORD) PIBYSIX_16 * 9;
...
//			SciaRegs.SCITXBUF=FlagSum;// & 0xff);
	...
	//	sum_cnt = a[3] + a[4] + a[5] + a[6] + a[7] + a[8];
		sum_cnt = a[0] + a[4] + a[8];// + a[6] + a[7] + a[8];

		/*---------------------------------------------------*/
		/* apply velocity = delta_theta/delta_time algorithm */
		/*---------------------------------------------------*/
		sum_cnt = K1_VELOCITY_EST / sum_cnt/2;
...
//		filt_velocity = (ALPHA * anSRM->wEst_10xrpm)+ (ONE_MINUS_ALPHA * inst_velocity);
	...
	//	anSRM->wEst_10xrpm = filt_velocity >> 3;
		anSRM->wEst_10xrpm = inst_velocity ;
	}
...
void PhaseControl(anSRM_struct *anSRM)	//int the ADC interrupt
{
	...
	for (phase = 0; phase< NUMBER_OF_PHASES; phase++)
	{
		...
		else if ((angle >= (182*25)) && (angle < 182*135))
		{
			anSRM->active[phase] = 1;
			temp = 0x1 << phase;
			anSRM->iDes[phase] = iDes;//Important!!
